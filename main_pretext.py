...
from src.utils.pretext_task import (
    get_center_block_mask,
    get_random_region_mask,
    generate_random_pattern,
    get_random_block_mask,
)
...
def main(args):
    ...
    if args.pretext_task_type == "jigsaw":
        ...
        assert os.path.exists(
            permutation_arr_path
        ), "no file or folder exists, use hamming_set.py to initialize the permutation_arr"

        permutation_arr_path_n_classes = permutation_arr_path.split(".")[0].split("_")[
            -1
        ]
        assert int(permutation_arr_path_n_classes) == config.model.get(
            "num_classes"
        ), "permutation_arr_path mismatch with num_classes"

    assert os.path.exists(
        args.unlabeled_datapath
    ), f"no file or folder exists at {args.unlabeled_datapath}"
    ...
    if args.use_validation:
        ...
        train_labels = [
            np.random.randint(0, 10) for _ in range(len(train_image_files_path))
        ]
        val_labels = [
            np.random.randint(0, 10) for _ in range(len(validation_image_files_path))
        ]
    ...
    if args.pretext_task_type == "context_encoder":
        ...
        context_generator = context_generator(
            bottleneck_dim=config.model.get("bottleneck_dim"),
            img_size=img_size,
            out_size=out_size,
            channels=3,
        )

        context_discriminator = getattr(
            networks, config.networks.get("discriminator_name")
        )
        context_discriminator = context_discriminator(
            input_size=out_size, in_channels=3
        )
    ...
    if args.pretext_task_type == "context_encoder":
        g_optimizer = get_optimizer(
            optim_type=config.optimizer.get("generator_type"),
            learning_rate=config.optimizer.get("generator_lr"),
        )
        d_optimizer = get_optimizer(
            optim_type=config.optimizer.get("discriminator_type"),
            learning_rate=config.optimizer.get("discriminator_lr"),
        )

    else:
        optimizer = get_optimizer(
            optim_type=config.optimizer.get("type"),
    ...
    if args.pretext_task_type == "context_encoder":
        ...
        reconstruction_criterion = get_criterion(
            config.criterion.get("reconstruction_type")
        )
    ...
    if args.pretext_task_type == "context_encoder":
        ...
        ckpt = tf.train.Checkpoint(
            step=tf.Variable(1), optimizer=optimizer, net=network
        )

    manager = tf.train.CheckpointManager(
        checkpoint=ckpt, directory=args.checkpoint, max_to_keep=3
    )
    ...
    if args.pretext_task_type == "context_encoder":
        ...
        top1_acc = tf.keras.metrics.SparseTopKCategoricalAccuracy(
            k=1, name="top_k_categorical_accuracy", dtype=None
        )
        top5_acc = tf.keras.metrics.SparseTopKCategoricalAccuracy(
    ...
    if args.use_validation:
        if args.pretext_task_type != "context_encoder":
            ...
            val_top1_acc = tf.keras.metrics.SparseTopKCategoricalAccuracy(
                k=1, name="top_k_categorical_accuracy", dtype=None
            )
            val_top5_acc = tf.keras.metrics.SparseTopKCategoricalAccuracy(
                k=5, name="top_k_categorical_accuracy", dtype=None
            )

        else:
            val_gen_recon_loss_tracker = tf.keras.metrics.Mean(
                name="gen_recon_loss_tracker"
            )

    # ----------------------------------
    # Load Summary Writer
    # ----------------------------------
    common_log_parent_path = (
        f"{args.tensorboard}/batch_level/"
    ...
    # ----------------------
    if config.model.get("random_masking"):
        global_random_pattern = generate_random_pattern(
            mask_area=config.model.get("mask_area"),
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.pretext_task_type == "context_encoder":
                ...
                if not config.model.get("random_masking"):
                    true_masks, masked_samples, _ = get_center_block_mask(
                        samples=samples.numpy(),
                        mask_size=mask_size,
                        overlap=config.model.get("overlap"),
                    )

                    masked_samples = tf.convert_to_tensor(
                        masked_samples, dtype=tf.float32
                    )
                ...
                values = [
                    ("gen_total_loss", batch_gen_total_loss),
        ...
        if args.use_validation:
            for val_step, val_batch in enumerate(val_dataloader):
                ...
                if args.pretext_task_type == "context_encoder":
                    ...
                    val_values = [
                        ("val_batch_gen_recon_loss", val_batch_gen_recon_loss)
                    ]
                    progbar.add(0, values=val_values)
                    ...
                    val_values = [
                        ...
                        ("val_top1_acc", val_batch_top1_acc),
                        ("val_top5_acc", val_batch_top5_acc),
                    ]
                    progbar.add(0, values=val_values)
        ...
        if args.use_validation:
            ...
            with val_writer.as_default(step=epoch):
                if args.pretext_task_type != "context_encoder":
                    ...
                    tf.summary.scalar(
                        "val_epoch_gen_recon_loss", val_epoch_gen_recon_loss
                    )
...
def train_context_encoder(
    ...
    with tf.GradientTape() as tape:
        ...
        gen_total_loss = (
            1 - config.model.get("r_weight")
        ) * gen_adv_loss + config.model.get("r_weight") * gen_recon_loss
    ...
    return {"gen_loss": gen_total_loss, "dis_loss": dis_loss}


def test_context_encoder(
    args, inputs, generator, recon_loss_func, val_gen_recon_loss_tracker
):
